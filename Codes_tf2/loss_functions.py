...
def gradient_loss(gen_frames, gt_frames, alpha):
    ...
    gen_dx = tf.abs(tf.nn.conv2d(gen_frames, filters=filter_x, strides=strides, padding=padding))
    gen_dy = tf.abs(tf.nn.conv2d(gen_frames, filters=filter_y, strides=strides, padding=padding))
    gt_dx = tf.abs(tf.nn.conv2d(gt_frames, filters=filter_x, strides=strides, padding=padding))
    gt_dy = tf.abs(tf.nn.conv2d(gt_frames, filters=filter_y, strides=strides, padding=padding))
