...
class Net(object):
    ...
    def train(self, log_dir, training_schedule, input_a, input_b, flow, checkpoints=None):
        ...
        if 'flow' in predictions:
            ...
            pred_flow_0 = tf.compat.v1.py_func(flow_to_image, [pred_flow_0], tf.uint8)
            pred_flow_1 = predictions['flow'][1, :, :, :]
            pred_flow_1 = tf.compat.v1.py_func(flow_to_image, [pred_flow_1], tf.uint8)
            pred_flow_img = tf.stack([pred_flow_0, pred_flow_1], 0)
            tf.compat.v1.summary.image('pred_flow', pred_flow_img, max_outputs=2)

        true_flow_0 = flow[0, :, :, :]
        true_flow_0 = tf.compat.v1.py_func(flow_to_image, [true_flow_0], tf.uint8)
        true_flow_1 = flow[1, :, :, :]
        true_flow_1 = tf.compat.v1.py_func(flow_to_image, [true_flow_1], tf.uint8)
