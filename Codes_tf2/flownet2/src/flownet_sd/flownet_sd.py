...
from tensorflow.keras import regularizers, initializers  # keras replacements


def l2_regularizer(scale):
    def regularizer(weights):
        return tf.multiply(scale, tf.nn.l2_loss(weights), name='l2_regularizer')
    return regularizer
    ...
    def model(self, inputs, training_schedule, trainable=True, reuse=None):
        ...
        with tf.compat.v1.variable_scope('FlowNetSD', reuse=reuse):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                                trainable=trainable,
                                # Weights initializer
                                weights_initializer=initializers.VarianceScaling(scale=2.0),
                                activation_fn=LeakyReLU,
                                padding='VALID'):

                # Weights regularizer
                weights_regularizer = l2_regularizer(0.5 * training_schedule['weight_decay'])
