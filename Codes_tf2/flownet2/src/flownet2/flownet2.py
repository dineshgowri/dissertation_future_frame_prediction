    ...
    def model(self, inputs, training_schedule, trainable=True):
        ...
        with tf.compat.v1.variable_scope('FlowNet2'):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                weights_regularizer = tf.keras.regularizers.l2(0.5 * (training_schedule['weight_decay']))
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    ...
                    flow = tf.image.resize(
                        predict_flow0, tf.stack([height, width]), method=tf.image.ResizeMethod.BILINEAR)
                    print(predict_flow0)
                    print(flow)
                    return {
