...
.Output("inv_spatial_transform_b: float32")
    ...
    with tf.compat.v1.Session() as sess:
        with tf.device('/gpu:0'):
            ...
            image_a_tf = tf.expand_dims(tf.cast(tf.constant(image_a, dtype=tf.float64), dtype=tf.float32), 0)
            image_b_tf = tf.expand_dims(tf.cast(tf.constant(image_b, dtype=tf.float64), dtype=tf.float32), 0)
            ...
            flow_tf = tf.expand_dims(tf.cast(tf.constant(flow), dtype=tf.float32), 0)
